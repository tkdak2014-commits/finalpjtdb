...
- 상단: 시스템 이름, 장비 연결 대기, 한국 시간, 사용자·권한, 로그아웃.
- 왼쪽 메뉴: 화면 내 지도·로봇 상태·영상·로그 영역으로 이동. 관리자만 사용자 관리 링크 표시.
- 지도: 등록 대기 영역과 배치 예시 도식. 실제 지도나 로봇 위치를 나타내지 않는다.
- 로봇 상태: AMR1·AMR2 카드, 배터리·임무·위치·최근 수신은 `—`로 표시.
- 영상: 로봇 1·2와 고정 웹캠 1·2의 영역 4개. 현재는 스트림 요청 없이 미연결 상태 표시.
- 이벤트 로그: 발생 시각·이벤트·감지 로봇·좌표·위험도·처리 상태·증거 이미지의 7개 열과 빈 목록 안내.
- 좁은 화면: 메뉴를 상단으로 옮기고 카드·영상을 세로로 배치. 이벤트 표만 가로 스크롤 가능.
